...
func main() {
	...
	fmt.Println("🚀 Inventory service on :8002")
	http.ListenAndServe(":8002", r)
}

func listInventory(w http.ResponseWriter, r *http.Request) {
	rows, err := db.Query("SELECT id, product_id, quantity, warehouse FROM inventory")
	if err != nil {
	...
	for rows.Next() {
		var inv Inventory
		rows.Scan(&inv.ID, &inv.ProductID, &inv.Quantity, &inv.Warehouse)
...
func getInventory(w http.ResponseWriter, r *http.Request) {
	...
	row := db.QueryRow("SELECT id, product_id, quantity, warehouse FROM inventory WHERE id = ?", id)

	var inv Inventory
	err := row.Scan(&inv.ID, &inv.ProductID, &inv.Quantity, &inv.Warehouse)
	if err != nil {
...
func getInventoryByProduct(w http.ResponseWriter, r *http.Request) {
	pid, _ := strconv.Atoi(chi.URLParam(r, "product_id"))
	row := db.QueryRow("SELECT id, product_id, quantity, warehouse FROM inventory WHERE product_id = ?", pid)
